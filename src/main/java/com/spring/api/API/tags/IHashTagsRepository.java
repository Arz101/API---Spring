package com.spring.api.API.tags;

import com.spring.api.API.posts.dtos.HashtagsDTO;
import com.spring.api.API.posts.dtos.HashtagsProjection;
...
public interface IHashTagsRepository extends JpaRepository<Hashtags, Long> {
    ...
    @Query("""
        SELECT new com.spring.api.API.posts.dtos.HashtagsDTO(h.name, ph.id)
        FROM Hashtags h
        JOIN h.posts ph
    """)
    ...
    @Query("""
        SELECT new com.spring.api.API.posts.dtos.HashtagsDTO(h.name, p.id)
        FROM Posts p
        JOIN p.hashtags h
