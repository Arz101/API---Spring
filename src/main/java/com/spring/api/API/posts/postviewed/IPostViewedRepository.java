package com.spring.api.API.posts.postviewed;

import com.spring.api.API.posts.dtos.PostViewedDto;
import com.spring.api.API.posts.postviewed.PostViewed.PostViewedId;
...
public interface IPostViewedRepository extends JpaRepository<PostViewed, PostViewedId>{
    ...
    @Query("""
        SELECT new com.spring.api.API.posts.dtos.PostViewedDto(
            pv.user.username,
            pv.user.id,
            pv.post.id
        )
